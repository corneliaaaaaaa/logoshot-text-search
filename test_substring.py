...
for index, row in df.iterrows():
    test_cases.append(row["substring"])
    answer_list.append(row["正確答案"])
    print(f"-----------------------start searching {row['substring']}------------------------")
    if constraints_mode:
...
test_df.to_csv(f"/home/ericaaaaaaa/logoshot/results/test_results/substring_{date}_{time}_c?{constraints_mode}_{accuracy: .3f}.csv", index=False)
